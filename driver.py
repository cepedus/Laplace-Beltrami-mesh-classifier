    ...
    for root, dirs, files in os.walk(models_path):
        for f in files:
            ...
            if model.split('.')[-1] == "obj":
                ...
                process = subprocess.Popen(["ssh", "-oStrictHostKeyChecking=no", username + "@" + node, "\"tmux", "new-session", "-s", "project_embeddings", "-d", "\\\"../project_bin", model, "\\\"\""], stdout=subprocess.PIPE, shell=True)
